<script>
  ...
  import { processModels } from './processModels.js'; // Datos simulados
  import { processModelDetailStore } from './processModelDetailStore.js';
  import ModelerNew from './ModelerNew.svelte'; // Componente para crear nuevo modelo (antes ModelerTest)

  const dispatch = createEventDispatcher();

  // Flag para mostrar el modal del modeler
  let isCreatingNew = false;

  // Copia local para que la UI sea reactiva al agregar nuevos elementos.
  // Si processModels es un array simple, copiamos sus elementos.
  // Si es otra cosa, inicializamos vacío (tu código original se respetó).
  let models = Array.isArray(processModels) ? [...processModels] : [];

  /**
   * Maneja el evento 'save' emitido por ModelerNew.
   * Recibe { xml } y crea un nuevo objeto modelo, lo agrega a la lista
   * y cierra el modal.
   */
  function handleSaveNew(event) {
